 ...
 * @var array
 */
	public $components = array('Paginator','FB');
 ...
 */
	...
	function userOrEmailAlreadyExist(){
		...
		if(!$data['Email']['address'] && !$error){
			$error['error']['msg'] = 'Email não pode ficar vazio';
		}

		if(!$data['User']['username'] && !$error){
			$error['error']['msg'] = 'Nome de usuario não pode ficar vazio';
		}

		if(!$data['User']['password'] && !$error){
			$error['error']['msg'] = 'O campo senha não pode ficar vazio';
		}

		if ($this->User->Email->findByAddress($data['Email']['address']) && !$error){
			 $error['error']['msg'] = 'Email já cadastrado';
		}

		if ($this->User->findByUsername($data['User']['username']) && !$error){
			 $error['error']['msg'] = 'Usuário já cadastrado';	
		}
 ...
 */
	...
	public function login(){
		...
		/*session_destroy();
		echo $this->FB->isUserLogged();		*/
		if ($this->Auth->login()){
			$this->redirect($this->Auth->redirect());
		}
        
        //$this->set('fb',$this->FB->getLoginURL());
	...
	function isLogged(){
		$data = array('User'=>array('username'=>'jonasporto','password'=>'jonasporto'));
	...
	public function add_fb_user() {
		...
		$user = array('User'=>array('username'=>$fb_user['username'],'password'=>$fb_user['username']));
	...
	public function loginByEmailOrUsername(){
    	...
    	if (!$this->Auth->login()) {
    		...
    		return json_encode(array('error' => array('msg' => 'Usuário ou senha incorretos !')));
    	}
